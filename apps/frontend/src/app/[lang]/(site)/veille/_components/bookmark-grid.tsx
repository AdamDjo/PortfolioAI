...
function BookmarkGrid({ bookmarks, allTags, canEdit }: BookmarkGridProps) {
  ...
  const { filter, grid } = getVeilleContent(locale)
  ...
  if (bookmarks.length === 0) {
    return <p className="veille-empty">{grid.emptyState}</p>
  }

  return (
    <>
      <div className="filter-row veille-tags" role="group" aria-label={filter.ariaLabel}>
        {tags.map((tag) => (
          <button
          ...
          >
            {tag === ALL_TAGS ? filter.allTag : tag}
          </button>
        ))}
      </div>
      ...
      {visibleBookmarks.length === 0 ? (
        <p className="veille-empty">{grid.emptyFilteredState}</p>
      ) : null}
    </>
  )
}
