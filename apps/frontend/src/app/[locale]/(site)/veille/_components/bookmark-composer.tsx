...
function BookmarkComposer({ tags }: BookmarkComposerProps) {
  ...
  async function addBookmark(raw: string) {
    ...
    if (!url) {
      setMessage(t('composerInvalidUrl'))
      return
    }
    ...
    try {
      ...
      if (response.status === 403 || response.status === 401) {
        setMessage(t('composerSessionExpired'))
        return
      }

      if (!response.ok) {
        // The most frequent case is the uniqueness violation on the URL.
        setMessage(t('composerSaveFailed'))
        return
      }
      ...
      setMessage(t('composerNetworkError'))
    } finally {
      setStatus('idle')
    }
  }
    ...
    <>
      <form className="search-bar veille-add-bar" onSubmit={submit}>
        ...
        <button type="submit" disabled={saving}>
          {saving ? (
            <>
              <Loader2 className="veille-spinner" size={14} /> {t('composerSubmitSaving')}
            </>
          ) : (
            <>
              <Plus size={14} /> {t('composerSubmitIdle')}
            </>
          )}
        </button>
      </form>
